...
class TransformerEncoderWrapper(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.transpose(0, 1)  # Transformer expects (seq_len, batch, feature)
        x = self.encoder(x)
        x = x.transpose(0, 1)  # Back to (batch, seq_len, feature)
        return x
...
class LagLlamaModel(nn.Module):
    def __init__(
        ...
        self.transformer = nn.ModuleDict(
            dict(
                ...
                #
                wte=TransformerEncoderWrapper(feature_size,config.n_embd_per_head * config.n_head,encoder_config),
    ...
    def prepare_input(
        ...
        # 这个patch序列将作为模型输入的一部分，用于捕捉时间序列数据中的上下文依赖关系。
        ...
        # 返回结果：
        # 如果提供了时间特征，则将patch序列、扩展后的静态特征和时间特征拼接起来，并返回这些特征、位置参数和尺度参数。
        if past_time_feat is not None:
            ...
            #如果没有提供时间特征，则只返回patch序列和扩展后的静态特征，以及位置参数和尺度参数。
